...
var recompute = function recompute(context) {
  ...
  var keys = allKeys.apply(void 0, _toConsumableArray(sources)); // Clean up properties that dont exists anymore
  ...
  try {
    var _loop2 = function _loop2() {
      ...
      if (isObject) {
        ...
        recompute(context, [].concat(_toConsumableArray(path), [key]), target[key], _keySources);
        return "continue";
...
var createProxy = function createProxy(context, target, resolveContext) {
  ...
  var proxy = markRaw(new Proxy(target, handler));
...
var createHandler = function createHandler(context, resolveContext) {
  ...
  return {
    get: function get(target, key, receiver) {
      ...
      if (!value[IS_PROXY]) {
        var keyPath = [].concat(_toConsumableArray(pathSegments), [key]);
...
function renderTag(context, key, data) {
  ...
  if (!attributes && _typeof(data) === 'object') {
  ...
  if (hasChilds) {
    ...
    if (!valueAttribute && contentAsAttribute) {
      var _getTagConfig = getTagConfig('attributes'),
          _getTagConfig2 = _slicedToArray(_getTagConfig, 1),
  ...
  var vnode = h(finalTag, attributes, content || undefined);
  return {
    to: data.to,
    vnode: vnode
  };
...
function renderAttributes(context, key, data, config) {
  ...
  if (context.isSSR) {
    ...
    return {
      to: '',
      vnode: h("ssr-".concat(attributesFor), data)
    };
  }
...
var PolySymbol = function PolySymbol(name) {
  return (// vm = vue meta
    hasSymbol ? Symbol("development" !== 'production' ? '[vue-meta]: ' + name : name) : ("development" !== 'production' ? '[vue-meta]: ' : '_vm_') + name
  );
};
var metaActiveKey = /*#__PURE__*/PolySymbol("development" !== 'production' ? 'meta_active' : 'ma');
...
function useMeta(source, manager) {
  var vm = getCurrentInstance() || undefined;
  ...
  if (isProxy(source)) {
    watch(source, function (newSource, oldSource) {
...
function addVnode(isSSR, teleports, to, vnodes) {
  ...
  (_teleports$to = teleports[to]).push.apply(_teleports$to, _toConsumableArray(nodes));
}
...
var MetaManager = /*#__PURE__*/function () {
  function MetaManager(isSSR, config, target, resolver) {
    ...
    _defineProperty(this, "isSSR", false);
    ...
    _defineProperty(this, "ssrCleanedUp", false);
  ...
  _createClass(MetaManager, [{
    ...
    key: "reallyUnmount",
    value: function () {
      var _reallyUnmount = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime.mark(function _callee(ignoreGuards, meta, metaGuards) {
...
_defineProperty(MetaManager, "create", function (isSSR, config, resolver) {
  ...
  return {
    created: function created() {
      ...
      if (isFunction(metaInfo)) {
        var computedMetaInfo = computed(metaInfo.bind(this));
